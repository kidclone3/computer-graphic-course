...
protected:
    ...
    void Add_Shader(const std::string vtx_shader_file_name, const std::string frag_shader_file_name,
    ...
    void
    Create_Plane_Mesh(const int m, const int n, const real dx, TriangleMesh<3> *mesh, int axis_0 = 0, int axis_1 = 1) {
    ...
    void Rescale(Array<Vector3> &vertices, const real longest_length) {
        ...
        real rescale = (length[axis] > (real) 0) ? longest_length / length[axis] : (real) 1;
        for (auto &v: vertices)v *= rescale;
    }
    ...
    void Create_Sphere_Mesh(const real r,/*rst*/TriangleMesh<3> *mesh, const int sub) {
        ...
        for (int i = 0; i < sub; i++)Subdivide(mesh);
    ...
    void Initialize_Icosahedron_Mesh(const real scale,/*rst*/TriangleMesh<3> *mesh) {
        ...
        for (int i = 0; i < vtx_num; i++) {
            mesh->Vertices()[i] = Vector3(vtx_pos[i][0], vtx_pos[i][1], vtx_pos[i][2]) * scale;
        }
        int ele_num = 20;
        mesh->elements.resize(ele_num);
        for (int i = 0; i < ele_num; i++)mesh->elements[i] = Vector3i(ele[i][0], ele[i][1], ele[i][2]);
    }
    ...
    void Subdivide(TriangleMesh<3> *mesh) {
        ...
        for (auto i = 0; i < n; i++) {
            ...
            int v3, v4, v5;
            {
                ...
                if (search == edge_vtx_hashtable.end())continue;
                v3 = search->second;
            }
            {
